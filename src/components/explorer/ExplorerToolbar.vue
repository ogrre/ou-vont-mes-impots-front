...
<template>
  <section class="explorer-toolbar" aria-label="Filtres de l’explorateur">
    ...
    <label class="control-field"><span>Mesure <GlossaryInfo term="CP" /><GlossaryInfo term="AE" /></span><select :value="measurement" @change="emit('update:measurement', ($event.target as HTMLSelectElement).value as 'payment_credit' | 'commitment_authorization')"><option value="payment_credit">CP — paiements</option><option value="commitment_authorization">AE — engagements</option></select></label>
    <label class="control-field"><span>Stade <GlossaryInfo term="Exécution" /><GlossaryInfo term="LFI" /></span><select :value="stage" @change="emit('update:stage', ($event.target as HTMLSelectElement).value as 'executed' | 'initial_budget')"><option value="executed">Exécuté</option><option value="initial_budget">Budget initial</option></select></label>
